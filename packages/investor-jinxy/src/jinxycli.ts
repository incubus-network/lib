...
import { JinxyApi } from './api'
import { jinxyAddresses } from './constants'
...
const main = async (): Promise<void> => {
  ...
  const jinxyContractAddress = jinxyAddresses[0].jinxy
  const jinxContractAddress = jinxyAddresses[0].jinx
  const jinxyStakingContractAddress = jinxyAddresses[0].staking
  const liquidityReserveContractAddress = jinxyAddresses[0].liquidityReserve
  ...
  const circulatingSupply = async () => {
    try {
      const supply = await api.tvl({ tokenContractAddress: jinxyContractAddress })
  ...
  const totalSupply = async () => {
    try {
      const supply = await api.totalSupply({ tokenContractAddress: jinxyContractAddress })
  ...
  const stakingTokenBalance = async () => {
    try {
      const balance = await api.balance({
        tokenContractAddress: jinxContractAddress,
        userAddress,
      })
  ...
  const rewardTokenBalance = async () => {
    try {
      const balance = await api.balance({
        tokenContractAddress: jinxyContractAddress,
        userAddress,
      })
    ...
    switch (index) {
      case 0:
        ...
        switch (tokenContract) {
          case 0:
            await approve(jinxContractAddress, jinxyStakingContractAddress)
            break
          case 1:
            await approve(jinxyContractAddress, jinxyStakingContractAddress)
            break
          default:
            break
        }
      ...
      case 1:
        ...
        switch (tokenContract) {
          case 0:
            await approve(jinxContractAddress, liquidityReserveContractAddress)
            break
          case 1:
            await approve(jinxyContractAddress, liquidityReserveContractAddress)
            break
          default:
            break
        }
