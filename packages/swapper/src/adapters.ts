...
const {
  ...
  UNCHAINED_BLACKFURY_HTTPS_API = 'http://localhost:31300',
  UNCHAINED_BLACKFURY_WS_API = 'wss://localhost:31300',
...
export const getAdapterManager = () => {
  ...
  const blackfuryAdapterArgs = {
    coinName: 'fury',
    providers: {
      ws: new unchained.ws.Client<unchained.blackfury.Tx>(UNCHAINED_BLACKFURY_WS_API),
      http: new unchained.blackfury.V1Api(
        new unchained.blackfury.Configuration({
          basePath: UNCHAINED_BLACKFURY_HTTPS_API,
        }),
      ),
    },
  }
